...
const init = () => {
    listData = (window as any).listData as IDataItem[];
    listEl = document.getElementById('listContainer')!;
    filterInputEl = document.getElementById('filterInput')! as HTMLInputElement;
...
const sort = (arr: IDataItem[]): IDataItem[] => {
    arr.sort((a, b) => {
        if (a.type == b.type) {
...
const filter = (arr: IDataItem[], query: string): IDataItem[] => {
    const q = query.toLocaleLowerCase().replace(regexp.filterQueryUnifier, '');
    
    return arr.filter(item => {
        const parts = [item.target];

        if (item.subtarget) parts.push(item.subtarget)
...
const render = (arr: IDataItem[], highlight: string|null = null) => {
...
const getHighlighedText = (text: string, highlight: string|null): HTMLElement => {
